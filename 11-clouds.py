...
def define_clouds_and_sky():
    clear_sky = pygame.image.load("images/clear_sky.png").convert_alpha()
    small_cloud = pygame.image.load("images/cloud_small.png").convert_alpha()
    large_cloud = pygame.image.load("images/cloud_large.png").convert_alpha()
    clouds = [small_cloud, large_cloud]
    return clouds, clear_sky

def generate_cloud(clouds):
    cloud_surf = random.choice(clouds)
    y_pos = random.randint(-50, 50)
    cloud_speed = random.randint(1, 3)
    cloud_rect = cloud_surf.get_rect(topleft=(800, y_pos))
    return (cloud_surf, cloud_rect, cloud_speed)
...
clouds_in_game = []
clouds, sky_surface = define_clouds_and_sky()
...
while True:
    ...
    for event in pygame.event.get():
        ...
        if game_active:
            ...
            if event.type == cloud_timer:
                clouds_in_game.append(generate_cloud(clouds))
            # Spawn enemies
    ...
    if game_active:
        ...
        # clouds
        render_clouds(clouds_in_game)
        clouds_in_game = move_clouds(clouds_in_game)
        # ground
